...
def send_alert_email1(to_email, month, income, expense_data, total_expense, limit):
    ...
    msg['From'] = 'triossoftwaremail@gmail.com'  
    msg['To'] = to_email

    try:
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as server:
            server.login('triossoftwaremail@gmail.com', 'knaxddlwfpkplsik')  
...
##            previous_total = emi + groceries + utilities + rent + transport + shopping + entertainment + savings
##            combined_total = previous_total + ocr_total
##
##            threshold_margin = 5000
##
...
##                            Savings: ₹{savings}
##
##                            🧾 New Expense from Your Receipt: ₹{ocr_total}
...
##                            rent, transport, shopping, entertainment, savings,
##                            receipt, total_expense
##                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
##                      (email, row[0], income, emi, groceries, utilities, rent,
##                       transport, shopping, entertainment, savings, str(ocr_total), combined_total))
##
##        conn.commit()
##
...
def upload():
    ...
    if file:
        ...
        """
            ...
            # Insert updated data into expenses_bill (remove receipt column)
            c.execute('''INSERT INTO expenses_bill (
                            email, month, income, emi, groceries, utilities,
...
# Define the folder for uploaded files and generated images
...
def send_alert_email(email, subject, body):
    ...
    with smtplib.SMTP('smtp.gmail.com', 587) as smtp:
        smtp.starttls()
        smtp.login('triossoftwaremail@gmail.com', 'knaxddlwfpkplsik')
        smtp.send_message(msg)


@app.route('/view_expenses')
def view_expenses():
    email = session.get('email')  # Or get from user input/admin role
    conn = sqlite3.connect(database)
    c = conn.cursor()
...
def chatbot_response(msg):
    ...
    for category in expense_categories:
        if category in messg:
            if category not in expense_keywords:
                ...
                print('expense_keywords:',expense_keywords)

    rent_match = re.search(r'\b\d+(?:,\d{3})*\b', msg)
    print('rent_match',rent_match)
    if 'home rent' in messg and rent_match:
        ...
        print('expense_amounts:',expense_amounts['home rent'])
    ...
    if 'emi' in messg and emi_match:
        ...
        print('expense_amountss:',expense_amounts['emi'])
        return f"Is there anything else? Please let me know, or Tell me your salary. I can suggest a plan."
    
    insurance_match = re.search(r'\b\d+(?:,\d{3})*\b', msg)
    print('insurance:', insurance_match)
    if 'insurance' in messg and insurance_match:
        ...
        print('expense_amountss:',expense_amounts['insurance'])
        return f"Is there anything else? Please let me know, or Tell me your salary. I can suggest a plan."

    loan_match = re.search(r'\b\d+(?:,\d{3})*\b', msg)
    print('loan:', loan_match)
    ...
    if salary_match and 'salary' in messg:
        salary = float(salary_match.group().replace(',', ''))  # Extract and convert salary amount
        ...
        print('home_rent:', home_rent)
        print('emi_rent:', emi_rent)
        print('insurance_rent:', insurance_rent)
        print('loan_rent:', loan_rent)
        print('groceries_rent:', groceries_rent)
        print('expecting_rent:', expecting_rent)

        remaining_salary = salary - (home_rent + emi_rent + loan_rent + insurance_rent + groceries_rent + expecting_rent + medical_rent)
        print('remaining_salary:', remaining_salary)
        ...
        print('split_amount:', split_amounts)
...
##    print("start")
##    speech = speak1()
##    result = chatbot_response(speech)  # your chatbot logic
##    response_audio_filename = f"response_{os.urandom(8).hex()}.mp3"
##    tts = gTTS(text=result, lang='en')
##    tts.save(os.path.join("static", response_audio_filename))
##    return jsonify({"speech": speech, "result": result, "audio_file": response_audio_filename})
